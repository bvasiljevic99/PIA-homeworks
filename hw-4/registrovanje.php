...
if($_SERVER["zahtev"] == "POST"){
	

	if(empty(trim($_POST["ime"]))){
        $ime_err = "potrebno je uneti ime.";
	} else{
        $ime = trim($_POST["ime"]);
    }
	...
	if(empty(trim($_POST["prezime"]))){
        $prezime_err = "potrebno je uneti prezime.";
	} else{
    ...
    if(empty(trim($_POST["email"]))){
        $email_err = "potrebno je uneti email adresu.";
    } else{
        $sql = "SELECT id_registrovani FROM ucesnici WHERE email = ?";
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_stmt_num_rows($stmt) == 1){
                    $email_err = "Ova email adresa vec postoji.";
                } else{
                    $email = trim($_POST["email"]);
                }
            } else{
                echo "Ups!Sistem javlja gresku.Pokusaj opet.";
            }
    ...
    if(empty(trim($_POST["korisnicko_ime"]))){
        $korisnicko_ime_err = " korisničko ime.";
    } else{
        $sql = "SELECT id_korisnika FROM ucesnici WHERE korisnicko_ime = ?";
    ...
    if(empty($ime_err) && empty($prezime_err) && empty($email_err) && empty($korisnicko_ime_err) && empty($lozinka_err)){
        ...
        $sql = "INSERT INTO ucesnici (ime, prezime, email, korisnicko_ime, lozinka) VALUES (?, ?, ?, ?, ?)";
...
<html>
	<head>
		...
		<link href="stilizacija.css" rel="stylesheet">
	</head>
<body>
    <div class="pocetna">
        <h2>Registrovanje</h2>
        <p>Potrebno je popuniti sve,radi registrovacije.</p>
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
            ...
            <p>Već ste registrovani? <a href="login.php">Onda se ulogujte ovde.</a>.</p>
        </form>
    </div>    
</body>
</html>
